...
static ArrayList<ServerList> serverList = new ArrayList<>();
	public static ServerList getServer() {
		// TODO Randome server assign
		Collections.shuffle(Server.serverList);
		System.out.println(Server.serverList.get(0).getMainPORT()+Server.serverList.get(0).getMainIP());
	...
	public static void main(String[] args) {
		...
		System.setProperty("java.rmi.server.hostname",DataContants.server2);
		...
		try {
			...
			LocateRegistry.createRegistry(DataContants.server2Port).rebind(DataContants.server2stub, stub);
